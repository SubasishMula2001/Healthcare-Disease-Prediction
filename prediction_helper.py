...
scaler_young = joblib.load("artifacts/scaler_young.joblib")  # dict with 'scaler' and 'cols_to_scale'
...
def calculate_normalized_risk(medical_history):
    ...
    max_score = 14  # 8 (heart disease) + 6 (diabetes or pressure)
    min_score = 0
...
def handle_scaling(age, df):
    ...
    # Add dummy 'income_level' column if needed
    if 'income_level' in cols_to_scale and 'income_level' not in df.columns:
        df['income_level'] = 0

    df[cols_to_scale] = scaler.transform(df[cols_to_scale])

    # Drop it after scaling to match model input
